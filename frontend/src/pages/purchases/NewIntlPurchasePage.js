...
import { intlPurchasesAPI, suppliersAPI, branchesAPI, scrapItemsAPI, companiesAPI, incotermsAPI, portsAPI, currenciesAPI } from '../../lib/api';
...
export default function NewIntlPurchasePage() {
  ...
  const loadData = async () => {
    try {
      const [compRes, branchRes, suppRes, itemRes, incRes, portRes, currRes] = await Promise.all([
        companiesAPI.list(),
        branchesAPI.list(),
        suppliersAPI.list('international'),
        scrapItemsAPI.list(),
        incotermsAPI.list(),
        portsAPI.list(),
        currenciesAPI.list()
  ...
  if (suppliers.length === 0) missingData.push('Suppliers (International)');
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center gap-4">
          <Button variant="ghost" onClick={() => navigate('/intl-purchases')}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">
                Before creating an international purchase order, you need to set up:
              </p>
              <ul className="mt-2 space-y-1 text-sm text-amber-700 dark:text-amber-400">
                {missingData.map(item => (<li key={item}>• {item}</li>))}
              </ul>
              <div className="mt-4 flex gap-2">
                <Button onClick={() => navigate('/suppliers')} variant="outline" size="sm">
                  Add International Supplier
                </Button>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="new-intl-purchase-page">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate('/intl-purchases')}>
          <ArrowLeft className="w-4 h-4" />
        </Button>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.company_id} onValueChange={(v) => setFormData({ ...formData, company_id: v })}>
                  <SelectTrigger className="form-input" data-testid="ipo-company-select">
              ...
              <div>
                ...
                <Select value={formData.branch_id} onValueChange={(v) => setFormData({ ...formData, branch_id: v })}>
                  <SelectTrigger className="form-input" data-testid="ipo-branch-select">
              ...
              <div>
                ...
                <Select value={formData.supplier_id || "select"} onValueChange={(v) => handleSupplierChange(v === "select" ? "" : v)}>
                  <SelectTrigger className="form-input" data-testid="ipo-supplier-select">
                    <SelectValue placeholder="Select supplier" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="select" disabled>Select supplier</SelectItem>
              ...
              <div>
                ...
                <Input type="date" value={formData.order_date} onChange={(e) => setFormData({ ...formData, order_date: e.target.value })} className="form-input" />
              </div>
              <div>
          ...
          <div className="kpi-card">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button onClick={addLine} variant="outline" size="sm" data-testid="ipo-add-line-btn">
          ...
          {/* Additional Costs */}
          <div className="kpi-card">
            <h3 className="fieldset-legend">Landed Cost Components</h3>
            <div className="grid grid-cols-4 gap-4">
              ...
              <div>
                ...
                <Input type="number" step="0.01" value={formData.insurance_cost} onChange={(e) => setFormData({ ...formData, insurance_cost: parseFloat(e.target.value) || 0 })} className="form-input" />
              </div>
              <div>
                <Label className="form-label">Customs Duty</Label>
                <Input type="number" step="0.01" value={formData.customs_duty} onChange={(e) => setFormData({ ...formData, customs_duty: parseFloat(e.target.value) || 0 })} className="form-input" />
              </div>
              <div>
                <Label className="form-label">Other Costs</Label>
                <Input type="number" step="0.01" value={formData.other_costs} onChange={(e) => setFormData({ ...formData, other_costs: parseFloat(e.target.value) || 0 })} className="form-input" />
              </div>
            </div>
          </div>
          ...
          <div className="kpi-card sticky top-24">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono">{formatCurrency(formData.insurance_cost, formData.currency)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Customs</span>
                <span className="font-mono">{formatCurrency(formData.customs_duty, formData.currency)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Other</span>
                <span className="font-mono">{formatCurrency(formData.other_costs, formData.currency)}</span>
              </div>
              <div className="border-t border-slate-200 dark:border-slate-700 pt-3 flex justify-between">
                <span className="font-bold">Landed Cost</span>
                <span className="font-mono font-bold text-lg">{formatCurrency(totals.landedCost, formData.currency)}</span>
              </div>
              <div className="flex justify-between text-sm text-slate-500">
                <span>In AED (@ {formData.exchange_rate})</span>
                <span className="font-mono">{formatCurrency(totals.landedCost * formData.exchange_rate, 'AED')}</span>
              </div>
            </div>
